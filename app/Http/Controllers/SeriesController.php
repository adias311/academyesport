...
class SeriesController extends Controller
{
    ...
    {
        // get all series
        $series = Series::with('videos')->orderBy('updated_at', 'asc')->get();
    ...
    public function show($slug)
    {
        ...
        $videos = Video::where('series_id', $series->id)->orderBy('episode', 'asc')->get();

        // call method members from trait HasSeries
        $members = $this->members($series)->count();

        // get transaction by user id
        $transaction = Transaction::with('details')->where('user_id', Auth::id())
        ->whereHas('details', function($query) use($series){
            $query->where('series_id', $series->id);
        })->first();

        return view('landing.series.show', compact('series','videos', 'members', 'transaction'));
    }
    ...
    public function video($slug, $episode)
    {
        ...
        $video = Video::where('series_id', $series->id)
        ->where('episode', $episode)
        ->whereNot('video_code', 'like', '%documents%')
        ->first();
        // get transaction by user id
    ...
    public function document($slug, $episode)
    {
        // get series by slug
        $series = Series::where('slug', $slug)->first();
        ...
        $video = Video::where('series_id', $series->id)
        ->where('episode', $episode)
        ->where('video_code', 'like', '%documents%')
        ->first();
        ...
        // user can watch full video if user have this series or user still can watch video only intro video
        if($purchased || $video->intro == 1 || Auth::user()->getRoleNames()[0] == 'admin'){
            // if true, get all video by series
            $videos = Video::where('series_id', $series->id)->orderBy('episode')->paginate(10);
        }else{
            // if false, get only intro video
        ...
        // return to view
        return view('landing.series.document', compact('series','video','videos'));
    }
}
