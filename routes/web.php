...
Route::controller(App\Http\Controllers\SeriesController::class)->prefix('series')->group(function(){
    ...
    Route::get('document/{series:slug}/{videos:episode}', 'document')->name('series.document')->middleware('auth');
});
// route carts
Route::controller(App\Http\Controllers\CartController::class)->prefix('carts')->middleware('auth')->group(function(){
    Route::get('/{series:id}', 'index')->name('carts.index');
    Route::post('/{series:slug}', 'store')->name('carts.store');
    Route::post('/store/data', 'stores')->name('carts.stores');
    Route::post('/store/subs', 'storeSubs')->name('carts.store.subs');
...
Route::post('transactions/store/subs', [App\Http\Controllers\TransactionController::class, 'store_subs'])
    ->name('transactions.store.subs')->middleware('auth');
// route comming soon
...
route::group(['prefix' => 'account', 'as' => 'member.', 'middleware' => ['auth']], function(){
    ...
    // route carts member
    Route::controller(App\Http\Controllers\Member\CartController::class)->prefix('cart')->group(function(){
...
Route::group(['prefix' => 'admin', 'as' => 'admin.',  'middleware' => ['auth', 'role:admin|instructor']], function(){
    ...
    // route documents
    Route::controller(App\Http\Controllers\Admin\DocumentController::class)->prefix('documents')->group(function(){
        // add documents by slug series
        Route::get('add/{series:slug}', 'create')->name('documents.create');
        Route::post('add/{series:slug}', 'store')->name('documents.store');
        // delete documents
        Route::delete('delete/{video:id}', 'destroy')->name('documents.destroy');
        // edit documents
        Route::get('edit/{video:id}', 'edit')->name('documents.edit');
        Route::put('update/{video:id}', 'update')->name('documents.update');
    });    // route videos
