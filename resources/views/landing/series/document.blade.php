...
@php
    $newPath = str_replace('public/documents/', 'storage/documents/', $video->video_code);
@endphp
...
@section('content')
    <div class="container-xl">
        <div class="row">
            <div class="col-12">
                <x-card.card title="Chapter {{ $video->episode }} : {{ $video->name }}">
                    <div class="embed-responsive embed-responsive-16by9">                        
                        <iframe class="embed-responsive-item" src="https://docs.google.com/gview?url={{ asset($newPath) }}&embedded=true"
                ...
                <x-card.card title="Chapter - {{ $video->episode }} {{ $video->name }}">
                    <div class="list-group list-group-flush">
                        @foreach ($videos->filter(fn($data) => str_contains($data->video_code, 'documents')) as $data)                            
                            <a href="{{ route('series.document', [$series->slug, $data->episode]) }}"
                                class="list-group-item list-group-item-action {{ request()->segment(4) == $data->episode ? 'active' : '' }}"
                                aria-current="true">
                                Chp - {{ $data->episode }} {{ $data->name }}
