        ...
        <div class="row">
            <div class="col-12">
                <x-card.card title="{{ $series->name }}">
                    <form action="{{ route('admin.documents.store', $series->slug) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <x-form.input type="text" title="Document Name" name="name" value=""
                            placeholder="Input document name" />                        
                        <x-form.input type="file" title="Upload Document" name="document" value=""
                            placeholder="" />                      
                        <div class="row">
                            <div class="col-6">
                                <x-form.input type="number" title="Document Number" name="number" value=""
